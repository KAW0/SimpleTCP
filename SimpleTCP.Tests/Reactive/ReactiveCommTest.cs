...
namespace SimpleTCP.Tests.Reactive
{
    ...
    public class ReactiveCommTest
    {
        ...
        {
            SimpleReactiveTcpServer server = new SimpleReactiveTcpServer().Start(8910);
            SimpleReactiveTcpClient client = new SimpleReactiveTcpClient().Connect(server.GetListeningIPs().FirstOrDefault(ip => ip.AddressFamily == System.Net.Sockets.AddressFamily.InterNetwork).ToString(), 8910);

            server.DelimiterDataReceived.Subscribe(msg =>
            {
            ...
            client.DelimiterDataReceived.Subscribe(msg =>
            {
        ...
        {
            SimpleReactiveTcpServer server = new SimpleReactiveTcpServer().Start(8911);
            server.Delimiter = Encoding.UTF8.GetBytes("0")[0];
            SimpleReactiveTcpClient client1 = new SimpleReactiveTcpClient().Connect(server.GetListeningIPs().FirstOrDefault(ip => ip.AddressFamily == System.Net.Sockets.AddressFamily.InterNetwork).ToString(), 8911);
            SimpleReactiveTcpClient client2 = new SimpleReactiveTcpClient().Connect(server.GetListeningIPs().FirstOrDefault(ip => ip.AddressFamily == System.Net.Sockets.AddressFamily.InterNetwork).ToString(), 8911);
